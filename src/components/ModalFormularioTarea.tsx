...
export const ModalFormularioTarea = () => {
    ...
    const [formulario, setFormulario] = useState<FormularioTareaState>({
        nombre: '',
        descripcion: '',
        fechaEntrega: '',
        prioridad: '',
    ...
    const { nombre, descripcion, fechaEntrega, prioridad } = formulario
    const { modalFormularioTarea, handleModalTarea, mostrarAlerta, alerta, submitTarea, tarea } = useProyectos();

    //* ----------------------------------------------------------------------------------------- *//

    useEffect(() => {
        if (tarea && typeof tarea === 'object' && '_id' in tarea) {
            setId(tarea._id || null);
            setFormulario({
                nombre: tarea.nombre,
